...
def _save_product(product: Product):
    ...
    try:
        product.price = int(float(request.form.get("price") or 0))
    except ValueError:
        abort(400, "Price must be a number.")
    original_price = request.form.get("original_price")
    try:
        product.original_price = int(float(original_price)) if original_price else None
    except ValueError:
        abort(400, "Original price must be a number.")
